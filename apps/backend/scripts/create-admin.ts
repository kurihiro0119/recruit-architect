...
async function main() {
  ...
  if (args.length < 3) {
    console.error('Usage: npx tsx apps/backend/scripts/create-admin.ts <email> <password> <name>');
    console.error('\nExample:');
    console.error('  npx tsx apps/backend/scripts/create-admin.ts admin@example.com password123 "Admin User"');
    process.exit(1);
  }
  ...
  try {
    ...
    const checkCommand = `wrangler d1 execute recruit-architect-db --local --command "SELECT id, email, name FROM admins WHERE email = '${escapedEmail}';"`;
    try {
      ...
      if (jsonStart >= 0) {
        ...
        if (Array.isArray(checkResult) && checkResult[0]?.results && checkResult[0].results.length > 0) {
      ...
      if (checkError.status === 0) {
        ...
        console.log('(Could not check for existing admin, proceeding anyway...)');
      }
    }
...
VALUES ('${id}', '${escapeSql(email)}', '${escapeSql(passwordHash)}', '${escapeSql(name)}', '${now}', '${now}');`;
    ...
    try {
      ...
      const command = `wrangler d1 execute recruit-architect-db --local --file=${tmpFile}`;
      ...
      console.log('\nVerifying...');
      const verifyCommand = `wrangler d1 execute recruit-architect-db --local --command "SELECT id, email, name FROM admins WHERE id = '${id}';"`;
      ...
      if (execError.status === 1) {
        ...
        console.error('  wrangler d1 execute recruit-architect-db --local --command "SELECT id, email, name FROM admins;"');
        process.exit(1);
      }
      throw execError;
    } finally {
